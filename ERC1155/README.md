...
The `PublicNFT` contract is an ERC-1155 compliant non-fungible token (NFT) collection that allows users to mint NFTs by paying a fixed price. Each minted token receives a dynamically generated token URI pointing to metadata stored on IPFS. The contract uses OpenZeppelin's standard implementations for ERC1155 functionality, URI storage, and ownership control.
...
- **Purpose:** Initializes the ERC-1155 token with name "PublicNFT1155" and symbol "PNFT1155"
...
- Create a new file: `PublicNFT1155.sol`
